...
import classes from "./css/CreatorForm.module.css";
...
export const manipulateCreatorAction = async ({ request, params }) => {
  ...
  if (method === "DELETE") {
    creatorObj.id = params.creatorId;
    console.log(url + `?id=eq.${creatorObj.id}`);
    response = await fetch(url + `?id=eq.${creatorObj.id}`, {
      headers: {
